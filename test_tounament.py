...
class nBall:
    ...
    def moveh(self, paddles, table_size, move_factor):
        moved = 0
        walls_Rects = [Rect((-100, -100), (table_size[0]+200, 100)),
                       Rect((-100, table_size[1]), (table_size[0]+200, 100))]
        for wall_rect in walls_Rects:
...
def pong_ai(paddle_frect, other_paddle_frect, ball_frect, table_size):
    ...
    spvb = curv
    mspvb = math.sqrt((spvb[0])**2+(spvb[1])**2)
#exist error in wall bounce? almost none with acc_predict 
    estpx,estpy = predict_position_acc(table_size,spvb,ball_frect)

    #注意，这个func没有被验证，请在beta1.0完成后验证
    ...
    bhy = true_estpy - paddle_frect.size[1]/2
    while bhy  <= true_estpy + paddle_frect.size[1]/2:

        '''
        new_test_us_paddle = fRect((paddle_frect.pos[0], bhy-paddle_frect.size[1]/2),paddle_frect.size)
        full_simulaiton(table_size,new_test_us_paddle, other_paddle_frect,spvb,ball_frect)
        # simvx,simvy,simvm,simpo = full_simulaiton(table_size,new_test_us_paddle, other_paddle_frect,spvb,ball_frect)
        #实际要根据模拟对打决定数值,太理想，跑不动 
        #事实证明貌似是因为while loop 太多。。。不过目前来看算法打的还不错,如果需要的话beta4.0改一下也可以
        '''

        #因为反弹貌似是双速*1.2，不论是比较速度还是落点都是不影响的，改变进攻法则的算法在击球之前决定就好
        #有可能写反fac，注意检查
        rd = (true_estpy - bhy)/70
        if spvb[0]>=0: #moving to the right
            pposx = 415
            fac = 0
            theta = rd*45*math.pi/180
        else:
            pposx = 15
            fac = 1
            theta = -1*rd*45*math.pi/180
        #注意：此处应该是下，y -- ysin2-xcos2, xsin2+ycos2
        #由于运算量级低，就直接cv了
        v = spvb
        v = [math.cos(theta)*v[0]-math.sin(theta)*v[1],
                math.sin(theta)*v[0]+math.cos(theta)*v[1]]
        ...
        # Bona fide hack: enforce a lower bound on horizontal speed and disallow back reflection
        if  v[0]*(2*fac-1) < 1 and v[1] !=0: # ball is not traveling (a) away from paddle (b) at a sufficient speed
            v[1] = (v[1]/abs(v[1]))*math.sqrt(abs(v[0]**2 + v[1]**2 - 1)) # transform y velocity so as to maintain the speed
            v[0] = (2*fac-1) # note that minimal horiz speed will be lower than we're used to, where it was 0.95 prior to increase by *1.2

        simvy = v[1]
        simvm = math.sqrt((v[0])**2 + (v[1])**2)
        wrongball = fRect((pposx,true_estpy), (15,15))
        simpox, simpo = predict_position_acc(table_size,v,wrongball)
        if mspvb >=3.9: #1.x,距离攻击
            #为避免换边
            if (simpo - 140) <= mopup: #尽可能上
                mopup = simpo - 140
                upbhy = bhy 
            if (simpo - 140) >= mopdn: #尽可能下
                mopdn = simpo - 140
                dnbhy = bhy 
            if other_paddle_frect.pos[1]<=140:
                estpy = dnbhy
            if other_paddle_frect.pos[1]>140:
                estpy = upbhy
        elif mspvb <= 18:#1.y,反弹攻击
            if simvy >= mysv:
                mysv = simvy 
                estpy = bhy
        elif mspvb >= 0:#1.z,线速度攻击
            if simvm >= mtsv:
                mtsv = simvm 
                estpy = bhy
    
        bhy += 7
        #check 发现进攻逻辑是有一些漏洞，会漏球，所以根据这个调整一下
        
        if estpy > true_estpy + 30:
            estpy = true_estpy + 30
        elif estpy < true_estpy - 30:
            estpy = true_estpy - 30
    ...
    #Alpha 2.0 is to be made only make precise defense when hitting to us 
        ...
        #实际上回中反倒是比较不错的手段，而且胜率很高。。。可以考虑一下怎么调整算法
...
'''
#这是一个完整模拟击球的程序，请小心output的值有四个
# 出球的x速度，y速度，总速度，敌方落点
# 请注意，本func应该在球朝我方时，其中paddle_frect应该是自变量
def full_simulaiton(table_size,paddle_frect, other_paddle_frect,pairvelocity,ball_frect):
    spx,spy =  pairvelocity
    ball = nBall(table_size, (15, 15), 1.2, 1, 0, pairvelocity, ball_frect.pos)

    paddles = [Paddle(paddle_frect.pos, paddle_frect.size, 1, 45,  1, 0.0003),
               Paddle(other_paddle_frect.pos, other_paddle_frect.size, 1, 45, 0, 0.0003),
               Paddle(paddle_frect.pos, paddle_frect.size, 1, 45,  0, 0.0003),
               Paddle(other_paddle_frect.pos, other_paddle_frect.size, 1, 45,  1, 0.0003)]
    ...
    #bug：没有回弹
    if spx > 0: #to the right  
        while ball.frect.pos[0] >=40:
            inv_move_factor = int((ball.speed[0]**2+ball.speed[1]**2)**.5)
            if inv_move_factor > 0:
                for i in range(inv_move_factor):
                    ball.moveh(paddles,table_size, 1./inv_move_factor)
            else:
                ball.moveh(paddles,table_size, 1)
        return ball.speed[0], ball.speed[1], ball.get_speed_mag(), ball_frect.pos[1]

    elif spx< 0: # to the left
        while ball.frect.pos[0] <= 400:
            inv_move_factor = int((ball.speed[0]**2+ball.speed[1]**2)**.5)
            if inv_move_factor > 0:
                for i in range(inv_move_factor):
                    ball.moveh(paddles,table_size, 1./inv_move_factor)
